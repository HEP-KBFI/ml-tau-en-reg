    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.cfg.version == "fine_tuning":
            ...
            print(
                "Training OmniParT head. Backbone parameters are unfrozen after initial epochs."
            )
        ...
        with torch.amp.autocast("cuda", enabled=self.use_amp):
            ...
            for block in self.blocks:
                cand_features_embed = block(
                    cand_features_embed, x_cls=None, padding_mask=padding_mask
                )
...
class EmbedParT(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self.vqvae_model = VQVAELightning.load_from_checkpoint(cfg.ckpt_path).to(
            device="cpu"
        )
        ...
        self.pp_dict = OmegaConf.to_container(
            ckpt_cfg.data.dataset_kwargs_common.feature_dict
        )
        n_GPT_blocks = 1
        ...
        if self.cfg.version != "from_scratch":
            ...
            gpt_state = {
                k.replace("module.", ""): v
                for k, v in loaded_bb_model["state_dict"].items()
    ...
    def forward(self, cand_omni_kinematics, cand_mask):
        ...
        cand_omni_kinematics[:, 0] = torch.nan_to_num(
            torch.log(cand_omni_kinematics[:, 0])
            - self.pp_dict["part_pt"]["subtract_by"]
            * self.pp_dict["part_pt"]["multiply_by"],
            nan=0.0,
            posinf=0.0,
            neginf=0.0,
        )
        ...
        x_particle_reco, vq_out = self.vqvae_model.forward(
            cand_omni_kinematics.permute(
                0, 2, 1
            ),  # To be in accordance with the order expected by the function
            torch.squeeze(cand_mask),  # Get rid of the axis=1
        )
        encoded_jets = self.bb_model(
            torch.squeeze(vq_out["q"], axis=2), padding_mask=torch.squeeze(cand_mask)
        )
        return encoded_jets

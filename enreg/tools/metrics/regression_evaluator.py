...
class RegressionEvaluator:
    def __init__(
        ...
        self.bin_edges = np.array(
            self.cfg.metrics.regression.ratio_plot.bin_edges[sample_name]
        )
    ...
    def _get_binned_values(self, ratios, gen_tau_vis_pt):
        ...
        binned_ratios = [
            ratios[binned_gen_tau_pt == bin_idx].to_numpy()
            for bin_idx in range(1, len(self.bin_edges))
        ]
        resolutions = np.array(
            [self.resolution_function(ratios) for ratios in binned_ratios]
        )
        responses = np.array(
            [self.response_function(ratios) for ratios in binned_ratios]
        )
...
class RangeContentPlot:
    def __init__(self, cfg, sample_name):
        ...
        self.bin_edges = np.array(
            self.cfg.metrics.regression.ratio_plot.bin_edges[self.sample_name]
        )
...
class Resolution2DPlot:
    ...
    def plot(self):
        fig, ax = plot_regression_confusion_matrix(
            ...
            left_bin_edge=0.0,
            right_bin_edge=self.cfg.metrics.regression.ratio_plot.bin_edges[
                self.sample
            ][-1],
            n_bins=24,
            figsize=(8, 9),

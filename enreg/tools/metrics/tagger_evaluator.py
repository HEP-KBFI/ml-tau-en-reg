...
class TaggerEvaluator:
    ...
    def _calculate_fakerates(self):
        ...
        # Denominator
        ref_var_pt_mask = (
            self.bkg_gen_jet_p4.pt > self.cfg.metrics.classifier.cuts.min_pt
        )
        ref_var_theta_mask1 = (
            abs(np.rad2deg(self.bkg_gen_jet_p4.theta))
            < self.cfg.metrics.classifier.cuts.max_theta
        )
        ref_var_theta_mask2 = (
            abs(np.rad2deg(self.bkg_gen_jet_p4.theta))
            > self.cfg.metrics.classifier.cuts.min_theta
        )
    ...
    def _calculate_efficiencies(self):
        ...
        # Denominator
        ref_var_pt_mask = (
            self.signal_gen_tau_p4.pt > self.cfg.metrics.classifier.cuts.min_pt
        )
        ref_var_theta_mask1 = (
        ...
        # Numerator
        tau_pt_mask = (
            self.signal_reco_jet_p4.pt > self.cfg.metrics.classifier.cuts.min_pt
        )
        tau_theta_mask1 = (
    ...
    def plot(self):
        ...
        ax.xaxis.set_major_locator(
            ticker.MultipleLocator(
                self.cfg.metrics.classifier.metrics[self.metric].x_maj_tick_spacing
            )
        )
        ax.set_xlabel(
            rf"{self.cfg.metrics.classifier.performances.fakerate.xlabel[self.metric]}",
            fontsize=30,
        )
        ax.set_ylabel(
            rf"{self.cfg.metrics.classifier.performances.fakerate.ylabel}", fontsize=30
        )
...
class EfficiencyPlot:
    ...
    def plot(self):
        ...
        ax.set_xlabel(
            rf"{self.cfg.metrics.classifier.performances.efficiency.xlabel[self.metric]}",
            fontsize=30,
        )
        ax.set_ylabel(
            rf"{self.cfg.metrics.classifier.performances.efficiency.ylabel}",
            fontsize=30,
        )
